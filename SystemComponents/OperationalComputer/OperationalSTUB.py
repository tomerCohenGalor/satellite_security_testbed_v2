...
from utils.OperationalResEx import cosmosData, command, commandAndCosmos
import random


# Retrieve manager socket IP and port from command-line arguments and configuration file
manager_socket_IP = sys.argv[1]
manager_socket_PORT = get_value_from_config_ini(
    "MANAGER_COM", "manager_client_PORT", "int"
)
    ...
    while True and i < maxNumberOfMsg:
        if i % 3 == 0:
            data_from_manager = client.execute(
                random.choice([cosmosData, command, commandAndCosmos])
            )
        else:
            data_from_manager = client.execute()
